{{-- // show chart by folder --}}
{{-- // add note in leeds
// chating with employee
// set call reminder --}}
                    ...
                    <div class="leave-boxone h-100">
                        ...
                        <div class="leace-conunt d-flex justify-content-between align-items-center mt-4">
                            <div class="leave-count-number">
                                @php
                                    $pp_target =
                                        $login_employee->monthly_sales_target / countDaysWithoutSundays($month, $year);

                                @endphp
                                <h4>{{ number_format($pp_target) }}</h4>
                                @if (isset($total_received))
                                    <p>Achived Target : <span class="text-success">+
                                            ₹{{ isset($total_received) ? number_format($today_received) : 0 }}</span></p>
                                    @if ($today_received < $pp_target)
                                        <p>Remaining Target : <span
                                                class="text-warning">₹{{ isset($total_received) ? number_format($pp_target - $today_received) : 0 }}</span>
                                        </p>
                                    @else
                            ...
                            <div class="laeave-ount-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em"
                                    height="1em" viewBox="0 0 32 32" data-icon="oui:app-users-roles" data-inline="false"
                                    style="font-size: 32px;" class="iconify iconify--oui">
                    ...
                    <div class="leave-boxone  h-100">
                        ...
                        <div class="leace-conunt d-flex justify-content-between align-items-center mt-4">
                            <div class="leave-count-number">
                                <h4>{{ $leads->count() }}</h4>
                                <p>Open leads : <a
                                        href="{{ route('office_employee.leads.index') }}?status=open&filter_by_lead={{ date('Y-m-d') }}"
                                        class="text-warning">{{ $leads->where('status', 'open')->count() }} <i
                                            class="fa-solid fa-link"></i></a></p>
                                <p>Converted leads : <a
                                        href="{{ route('office_employee.leads.index') }}?status=converted&filter_by_lead={{ date('Y-m-d') }}"
                                        class="text-success">{{ $leads->where('status', 'converted')->count() }} <i
                                            class="fa-solid fa-link"></i></a></p>
                            </div>
                            <div class="laeave-ount-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em"
                                    height="1em" viewBox="0 0 24 24" data-icon="iconoir:rocket" data-inline="false"
                                    style="font-size: 32px;" class="iconify iconify--iconoir">
                        ...
                        <div class="leace-conunt d-flex justify-content-between align-items-center mt-4">
                            <div class="leave-count-number">
                                ...
                                <p>
                                    ...
                                    <a href="{{ route('office_employee.leads.index') }}?status={{$st}}"
                                    class="badge {{$key}}"> {{$st}}
